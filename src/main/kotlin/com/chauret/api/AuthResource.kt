    ...
    override fun intercept(
        ...
        val power = runCatching {
            SessionService.getSessionByGuid(UUID.fromString(request.headers[AUTH_HEADER]))?.username?.let {
                UserService.getByUsername(it).permissions.power
            } ?: Permissions.USER.power
        }.getOrElse { 0 }
        Permissions.values().forEach { permission ->
            if (request.method != HttpMethod.OPTIONS && key.path.toAbsoluteString()
                    .contains(permission.name.lowercase()) && power < permission.power
            ) {
